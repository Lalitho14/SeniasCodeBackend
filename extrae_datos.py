...
def GuardarKeypointsH5(_sequences, _labels, _out_folder):
...
def ProcesarFrame(frame):
  ...
  keypoints_normalizados = NormalizarKeypoints(keypoints)
  #VerificarFrame(frame, results)
...
def NormalizarKeypoints(keypoints_array):
  # Suponiendo que los primeros 33*3 valores son pose (índice 0 es nariz)
  reference_x, reference_y, reference_z = keypoints_array[0], keypoints_array[1], keypoints_array[2]
...
def Extraer_Keypoints(in_ruta, out_ruta):
...
def GraficarKeypoints(array_keypoints, title):
  plt.scatter(array_keypoints[::3], array_keypoints[1::3], s=1)
  plt.title(title)
  plt.show()


def InicializarHolistic():
...
if __name__ == "__main__":
  ...
  Extraer_Keypoints(in_folder, out_folder)
